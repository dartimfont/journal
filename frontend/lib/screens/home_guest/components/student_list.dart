...
class _StudentListState extends State<StudentList> {
  ...
  Future<List<Student>> fetchJson() async {
    dynamic params = jsonEncode({"group": globals_student.group});
    dynamic response =
          ...
          child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                return GestureDetector(
                  onTap: () {
                    setState(() {
                      globals_student.selectedIndexInStudents = index;
                      globals_student.id_student = _students[index].id_student;
                      globals_student.surname = _students[index].surname;
                      globals_student.name = _students[index].name;
                    });
                  },
                  child: Container(
                    margin: EdgeInsets.symmetric(vertical: 4),
                    padding: EdgeInsets.symmetric(vertical: 8),
                    color: index == globals_student.selectedIndexInStudents
                        ? Colors.black12
                        : Colors.white60,
                    child: Row(
                      children: [
